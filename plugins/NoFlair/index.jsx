...
`;
  ...
  const GRAD_SELECTOR =
    '[style*="--custom-gradient-color-1"], [style*="--custom-gradient-color-2"], [style*="--custom-gradient-color-3"]';
  const GRAD_CLASS_SELECTOR =
    '[class*="twoColorGradient"], [class*="usernameGradient"], [class*="convenienceGlowGradient"]';
  ...
  const averageLinear = (colors) => {
    ...
    for (const { r, g, b, a } of colors) {
      rl += toLin(r / 255); gl += toLin(g / 255); bl += toLin(b / 255); al += a;
    }
  ...
  shelter.solid.createRoot((dispose) => {
    ...
    shelter.solid.createEffect(() => {
      if (unObs)      { try { unObs.now?.(); } catch {} unObs = undefined; }
      if (unSolidCss) { try { unSolidCss(); } catch {} unSolidCss = undefined; }
