...
const LOCALES = {
	en: {
		...
		vanillaNick_note: "Removes nitro name styles.",
	...
	ru: {
		...
		vanillaNick_note: "Убирает донатные стили ников.",
...
`;
	...
	function neutralizeNickname(node) {
		const el = node;
		if (!el) return;
		if (el.hasAttribute?.("data-username-with-effects")) el.removeAttribute("data-username-with-effects");
		for (const p of [
			"background",
			"background-image",
			"text-shadow",
		...
		for (const c of [...el.classList]) if ([
			"gradient_",
			"toon_",
			"underlineOnHover_",
			"animated_",
			...
			"cherryBomb",
			"sinistre"
		].some((b) => c.includes(b))) el.classList.remove(c);
	}
	const TAG_SELECTORS = [
		"[class^=\"guildTag_\"]",
		"[class*=\" guildTag_\"]",
		"[class^=\"guildTagContainer_\"]",
		"[class*=\" guildTagContainer_\"]",
		"[class^=\"clanTag_\"]",
		"[class*=\" clanTag_\"]",
		"[class^=\"clanTagChiplet_\"]",
		"[class*=\" clanTagChiplet_\"]",
		"[aria-label^=\"Server Tag\"]",
		"[aria-label*=\"Server Tag:\"]",
		"[aria-label^=\"Тег сервера\"]",
		"[aria-label*=\"Тег сервера:\"]",
		"img[src*=\"/clan-badges/\"]"
	].join(",");
	...
	shelter.solid.createRoot((dispose) => {
		shelter.solid.createEffect(() => {
			...
			moTags = new MutationObserver((muts) => {
				for (const m of muts) if (m.type === "childList") m.addedNodes.forEach((n) => {
					...
					if (al.startsWith("Server Tag") || al.startsWith("Тег сервера")) hideGuildTagNode(target);
				}
			});
...
function onUnload() {}
function settings() {
	const { plugin: { store }, ui: { SwitchItem, Divider }, solidH: { html } } = shelter;
	return html`
    <${SwitchItem}
